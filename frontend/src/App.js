...
const resetPasswordUtil = (password, email) => {
  axios
    .put(`/api/members/update/password_for_user/${email}`, {
      password
    })
    .then(({ data }) => {
      message.ask({
  ...
  return (
    ...
    >
      <Wrapper visible={true}>
        <h4>ajouter un nouveau mot de passe</h4>
      </Wrapper>
      ...
      <Wrapper visible={true}>
        <Input
          ...
          onChange={evt => {
            ...
            store.setState(state => ({ ...state, password }));
          }}
          type={"password"}
        />
      </Wrapper>
      ...
      <Wrapper visible={true}>
        <Input
          ...
          onChange={evt => {
            ...
            store.setState(state => ({ ...state, confirm }));
          }}
          type={"password"}
        />
      </Wrapper>
...
const LoadError = props => (
  ...
  >
    <h4>
      Désolé, le lien reste mot de passe est expiré s'il vous plaît essayez à
      nouveau
    </h4>
    <a href="/reset/password">réessayer</a>
  </div>
);
...
class LoadOrFail extends React.Component {
  ...
  componentDidMount() {
    axios
      .post(`/api/members/verifyEmail/${this.props.token}`)
      .then(({ data: { isValid } }) => {
  ...
  render() {
    ...
    return (
      <div>
        {isValid ? (
          <LoadForm store={this} email={this.props.email} />
        ) : (
          <LoadError />
        )}
      </div>
